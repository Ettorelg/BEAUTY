...
export async function sendBookingConfirmation({email,businessName,serviceName,startsAt,timezone}:{email:string;businessName:string;serviceName:string;startsAt:Date;timezone:string}) { const apiKey=process.env.RESEND_API_KEY,from=process.env.RESEND_FROM_EMAIL; if(!apiKey||!from)return; const when=startsAt.toLocaleString("it-IT",{dateStyle:"long",timeStyle:"short",timeZone:timezone}); await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({from,to:[email],subject:`Conferma prenotazione · ${businessName}`,html:`<p>La tua prenotazione è confermata.</p><p><strong>${businessName}</strong><br/>${serviceName}<br/>${when}</p>`})}); }