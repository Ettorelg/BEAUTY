...
export function ensureFidelitySchema(){
  ...
  ready=(async()=>{
    ...
    try{
      ...
      await client.query('ALTER TABLE "fidelity_settings" ADD COLUMN IF NOT EXISTS "reward_service_id" uuid REFERENCES "services"("id") ON DELETE SET NULL');await client.query('ALTER TABLE "fidelity_settings" ADD COLUMN IF NOT EXISTS "points_validity_months" integer NOT NULL DEFAULT 12');await client.query('ALTER TABLE "fidelity_settings" ADD COLUMN IF NOT EXISTS "allow_reward_stacking" boolean NOT NULL DEFAULT false');
      await client.query('ALTER TABLE "fidelity_settings" ADD COLUMN IF NOT EXISTS "award_mode" text NOT NULL DEFAULT \'BY_SPEND\'');
      ...
      await client.query('CREATE UNIQUE INDEX IF NOT EXISTS "fidelity_cards_number_unique" ON "fidelity_cards" ("card_number")');await client.query('ALTER TABLE "fidelity_cards" ADD COLUMN IF NOT EXISTS "points_expires_at" timestamptz');
      ...
      await client.query('CREATE INDEX IF NOT EXISTS "fidelity_redemptions_business_customer_idx" ON "fidelity_redemptions" ("business_id","customer_relation_id")');await client.query('ALTER TABLE "fidelity_redemptions" ADD COLUMN IF NOT EXISTS "appointment_id" uuid REFERENCES "appointments"("id") ON DELETE SET NULL');await client.query('ALTER TABLE "fidelity_redemptions" ADD COLUMN IF NOT EXISTS "reversed_at" timestamptz');await client.query('CREATE UNIQUE INDEX IF NOT EXISTS "fidelity_redemptions_appointment_unique" ON "fidelity_redemptions" ("appointment_id") WHERE "appointment_id" IS NOT NULL');
