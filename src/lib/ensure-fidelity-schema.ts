...
export function ensureFidelitySchema() {
  ...
  ready = (async () => {
    ...
    try {
      ...
      await client.query(
        'CREATE TABLE IF NOT EXISTS "fidelity_settings" ("business_id" uuid PRIMARY KEY REFERENCES "businesses"("id") ON DELETE CASCADE,"spend_cents" integer NOT NULL DEFAULT 1000,"points_award" integer NOT NULL DEFAULT 1,"reward_points" integer NOT NULL DEFAULT 10,"reward_type" text NOT NULL DEFAULT \'DISCOUNT_EUR\',"reward_value" integer NOT NULL DEFAULT 500,"reward_service_id" uuid REFERENCES "services"("id") ON DELETE SET NULL,"created_at" timestamptz NOT NULL DEFAULT now(),"updated_at" timestamptz NOT NULL DEFAULT now())',
      );
      await client.query(
      ...
      await client.query(
        'ALTER TABLE "fidelity_settings" ADD COLUMN IF NOT EXISTS "award_mode" text NOT NULL DEFAULT \'BY_SPEND\'',
      );
      await client.query(
      ...
      await client.query(
        'CREATE TABLE IF NOT EXISTS "promotion_broadcasts" ("id" uuid PRIMARY KEY DEFAULT gen_random_uuid(),"business_id" uuid NOT NULL REFERENCES "businesses"("id") ON DELETE CASCADE,"promotion_id" uuid REFERENCES "fidelity_promotions"("id") ON DELETE SET NULL,"status" text NOT NULL DEFAULT \'SENDING\',"recipient_count" integer NOT NULL DEFAULT 0,"success_count" integer NOT NULL DEFAULT 0,"failure_count" integer NOT NULL DEFAULT 0,"last_error" text,"sent_at" timestamptz,"created_at" timestamptz NOT NULL DEFAULT now())',
      );
      await client.query(
        'CREATE INDEX IF NOT EXISTS "promotion_broadcasts_business_idx" ON "promotion_broadcasts" ("business_id")',
      );
      await client.query(
