...
export default async function ServicesPage() {
  ...
  const catalog = await db.select({
    ...
    price: services.price, repeatPrice: services.repeatPrice, repeatPriceEnabled: services.repeatPriceEnabled, repeatDurationMinutes: services.repeatDurationMinutes, onlineBookable: services.onlineBookable, categoryName: serviceCategories.name,
  ...
  return <main className="dashboard-shell">
    ...
    <section className="management-grid compact-management-grid">
      <details className="panel action-disclosure"><summary><span><b>Nuova categoria</b><small>Crea e organizza le categorie</small></span><i>＋</i></summary><div className="action-disclosure-body"><form action={createCategory} className="compact-form"><input name="name" placeholder="Es. Capelli" required minLength={2}/><button className="primary-button">Aggiungi</button></form><div className="category-list"><p className="eyebrow">Categorie inserite</p>{categories.length ? <div className="category-chips">{categories.map(category => <span className="category-chip" key={category.id}>{category.name}</span>)}</div> : <p className="muted">Nessuna categoria ancora.</p>}</div></div></details>
      <details className="panel action-disclosure"><summary><span><b>Nuovo servizio</b><small>Aggiungi una voce al listino</small></span><i>＋</i></summary><div className="action-disclosure-body">{categories.length ? <form action={createService} className="compact-form stacked">
        ...
        <div className="form-row"><label>Durata (min)<input name="durationMinutes" type="number" min={5} step={5} defaultValue={30} required/></label><label>Prezzo prima prenotazione (€)<input name="price" type="number" min={0} step="0.01" required/></label><label>Prezzo dalla seconda (€)<input name="repeatPrice" type="number" min={0} step="0.01" placeholder="Prezzo scontato"/></label><label>Durata dalla seconda (min)<input name="repeatDurationMinutes" type="number" min={5} max={480} step={5} placeholder="Uguale alla prima"/></label></div><label className="checkbox-row"><input name="repeatPriceEnabled" type="checkbox"/> Abilita prezzo diverso dalla seconda prenotazione</label>
