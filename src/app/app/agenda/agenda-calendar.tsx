...
import {
  ...
  monthGridDates,
  type AgendaView,
} from "@/modules/agenda/domain/calendar";
import {
  addCustomProductToAppointment,
  addProductToAppointment,
  addServiceToAppointment,
  approveCustomerRescheduleRequestSafely,
...
type Data = {
  ...
  additionalServiceCatalog: Array<{
    id: string;
    name: string;
    duration: number;
    price: string;
...
function ProductPanel({
  ...
  const available = data.inventoryCatalog.filter(
    (product) => product.stock > 0,
  );
  const suggested = available.filter((product) =>
    entry.recommendedProductIds.includes(product.id),
  );
  ...
  return (
    <section className="agenda-action-panel agenda-products-panel">
      ...
      {data.usedProducts.filter((item) => item.appointmentId === entry.id)
        .length ? (
        <div className="agenda-product-list">
          {data.usedProducts
            .filter((item) => item.appointmentId === entry.id)
            .map((item) => (
              <small key={item.id}>
                <span>
                  {item.name} × {item.quantity}
                </span>
                <span className="agenda-product-line-actions"><strong>{money(Number(item.unitPrice) * item.quantity)}</strong><button type="button" title="Rimuovi articolo" aria-label={`Rimuovi ${item.name}`} onClick={async()=>{const form=new FormData();form.set("id",item.id);try{await removeProductFromAppointment(form);await load();}catch(error){setError(error instanceof Error?error.message:"Impossibile rimuovere l’articolo.")}}}>×</button></span>
              </small>
            ))}
        </div>
      ) : (
        <small className="muted">Nessun articolo ancora associato.</small>
      )}
      {suggested.length ? (
        <>
          <span className="eyebrow">Consigliati per questo servizio</span>
          <div className="agenda-product-preview">
            {suggested.map((product) => (
              <button
                ...
                onClick={async () => {
                  ...
                  f.set("appointmentId", entry.id);
                  f.set("productId", product.id);
                  f.set("quantity", "1");
                  try {
                    await addProductToAppointment(f);
                    await load();
                  } catch {
                    setError(
                      "Impossibile aggiungere il prodotto: controlla la giacenza.",
                    );
                  }
                }}
              >
      ...
      <label>
        ...
        <input
          ...
          placeholder="Nome o categoria del prodotto"
        />
      </label>
      {options.length ? (
        <form
          action={async (formData) => {
            try {
              await addProductToAppointment(formData);
              await load();
            } catch {
              setError(
                "Impossibile aggiungere il prodotto: controlla la giacenza.",
        ...
        >
            ...
            <select name="productId">
              {options.map((product) => (
                <option key={product.id} value={product.id}>
                  {product.name} · {product.categoryName ?? "Senza categoria"} ·
          ...
          <button className="primary-button">Aggiungi alla prenotazione</button>
        </form>
      ) : query.trim() ? (
        <small className="muted">Nessun articolo trovato.</small>
      ) : null}
      <button
        type="button"
      ...
      {custom ? (
        <form
          action={async (formData) => {
            try {
              await addCustomProductToAppointment(formData);
              await load();
              setCustom(false);
            } catch {
              setError("Controlla descrizione e importo dell’articolo vario.");
            }
          }}
          className="compact-form stacked"
        >
          ...
          <label>
            ...
            <input
              name="description"
              placeholder="Es. Accessorio o prodotto non catalogato"
              required
            />
          </label>
          ...
          <button className="primary-button">Aggiungi articolo vario</button>
        </form>
      ) : null}
      {!available.length ? (
        <div className="agenda-stock-empty">
          <strong>Articoli di magazzino non disponibili</strong>
          <p>
            {data.inventoryCatalog.length
              ? "Gli articoli presenti hanno giacenza zero."
              : "Non hai ancora creato articoli."}
          </p>
          <a className="ghost-button link-button" href="/app/inventory">
            Vai al Magazzino e registra un carico
          </a>
        </div>
      ) : null}
    </section>
...
function AdditionalServiceForm({
  ...
  const first = data.additionalServiceCatalog[0];
  const [serviceId, setServiceId] = useState(first?.id ?? "");
  const [duration, setDuration] = useState(first?.duration ?? 30);
  const [price, setPrice] = useState(first?.price ?? "0");
  if (!first)
    return <p className="muted">Nessun altro servizio disponibile.</p>;
  return (
    <div className="agenda-extra-service">
      ...
      <form
        action={async (formData) => {
          try {
            await addServiceToAppointment(formData);
            await load();
          } catch (error) {
            setError(
      ...
      >
        ...
        <label>
          ...
          <select
            ...
            value={serviceId}
            onChange={(event) => {
              const selected = data.additionalServiceCatalog.find(
          ...
          >
            {data.additionalServiceCatalog.map((service) => (
...
function EditServiceForm({ data, entry }: { data: Data; entry: Entry }) {
  const unique = Array.from(
      new Map(data.catalog.map((item) => [item.id, item])).values(),
    ),
    current = unique.find((item) => item.id === entry.serviceId) ?? unique[0],
    categories = Array.from(
      new Map(
        unique.map((item) => [item.categoryId, item.categoryName]),
      ).entries(),
    ),
    [category, setCategory] = useState(
      current?.categoryId ?? categories[0]?.[0] ?? "",
    ),
    filtered = unique.filter((item) => item.categoryId === category),
    [serviceId, setServiceId] = useState(current?.id ?? ""),
    [duration, setDuration] = useState(current?.duration ?? 30);
  return (
    <form action={changeAppointmentService} className="compact-form stacked">
        ...
        <select
          ...
          onChange={(event) => {
            const id = event.target.value,
              first = unique.find((item) => item.categoryId === id);
            setCategory(id);
            setServiceId(first?.id ?? "");
            setDuration(first?.duration ?? 30);
          }}
        >
      ...
      <label>
        ...
        <div className="duration-stepper">
          ...
          <button
            type="button"
            onClick={() => setDuration((value) => Math.min(480, value + 5))}
          >
            ＋
          </button>
        </div>
      </label>
      <button className="ghost-button">Salva servizio e durata</button>
    </form>
  );
}
...
function Booking({
  ...
  const categories = Array.from(
    new Map(
      uniqueServices.map((item) => [item.categoryId, item.categoryName]),
    ).entries(),
  );
  const [category, setCategory] = useState(categories[0]?.[0] ?? "");
  const categoryServices = uniqueServices.filter(
  ...
  const eligible = data.staff.filter((member) =>
    data.catalog.some(
      (item) => item.id === service && item.staffId === member.id,
    ),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(
      `/api/agenda/availability?serviceId=${service}&date=${day}&staffId=${staff}&durationMinutes=${duration}`,
    )
        ...
        <form className="compact-form stacked" onSubmit={submit}>
          <label>
            ...
            <select
              ...
              onChange={(event) => {
                ...
                const first = uniqueServices.find(
                  (item) => item.categoryId === nextCategory,
                );
          ...
          <label>
            ...
            <select
              value={service}
              onChange={(event) => {
                const next = uniqueServices.find(
                  (item) => item.id === event.target.value,
                );
                setService(event.target.value);
                setDuration(next?.duration ?? 30);
                setStaff("");
              }}
            >
          ...
          <div className="slot-picker">
            {loading ? (
              <p className="muted">Cerco disponibilità…</p>
            ) : slots.length ? (
              slots.map((slot) => (
                <button
                  type="button"
                  className={
                    selected?.localStart === slot.localStart &&
                ...
                >
                  {slot.label}
                  {!staff ? <small> · {slot.staffName}</small> : null}
                </button>
              ))
            ) : (
          ...
          {selected ? (
            <>
              <p className="status-pill">
                {selected.label} · {selected.staffName}
              </p>
              <CustomerAutofill />
              <input
                type="hidden"
...
export function AgendaCalendar({ today }: { today: string }) {
  ...
  const [completionPayment, setCompletionPayment] = useState<"PAID" | "UNPAID">(
    "PAID",
  );
  ...
  const load = useCallback(async () => {
    try {
      const response = await fetch(`/api/agenda?date=${date}&view=${view}`, {
        cache: "no-store",
      });
  ...
  useEffect(() => {
    router.replace(`/app/agenda?date=${date}&view=${view}`, { scroll: false });
  }, [date, view, router]);
  ...
  const expected = useMemo(() => {
    ...
    const rows =
      data?.entries.filter(
        (entry) =>
          visibleStaffIds.has(entry.staffId) &&
    ...
    return {
      ...
      byStaff: (data?.staff ?? []).map((member) => ({
        ...
        total: rows
          ...
          .reduce((sum, entry) => sum + amount(entry.price), 0),
      })),
    };
  }, [data]);
    ...
    data.view === "year"
      ...
      : data.view === "month"
        ? new Intl.DateTimeFormat("it-IT", {
            ...
            year: "numeric",
            timeZone: "UTC",
          }).format(new Date(`${data.startDate}T12:00:00Z`))
        : new Intl.DateTimeFormat("it-IT", {
            weekday: "long",
            day: "numeric",
            month: "long",
    ...
    <>
      <div className="agenda-toolbar">
        ...
        <div className="agenda-view-switch">
          {data.canManage ? (
            ...
            >
              {showRevenue ? "Nascondi incasso" : "Incasso previsto"}
            </button>
          ) : null}
          <button
      ...
      {data.rescheduleRequests?.length ? (
        <section className="panel">
          ...
          {data.rescheduleRequests.map((request) => (
            <article className="data-row" key={request.id}>
              ...
              <div className="button-row">
                <button
                  ...
                  type="button"
                  disabled={reschedulePending === request.id}
                  onClick={() => void decideReschedule(request.id, "approve")}
                >
                  {reschedulePending === request.id ? "Verifica…" : "Accetta"}
                </button>
                <button
                  className="danger-button"
                  type="button"
                  disabled={reschedulePending === request.id}
                  onClick={() => void decideReschedule(request.id, "reject")}
                >
                  Rifiuta
                </button>
              </div>
      ...
      {data.canManage && showRevenue ? (
        <section className="agenda-revenue-card">
          <div>
            <p className="eyebrow">Incasso previsto</p>
            <strong>{money(expected.total)}</strong>
            <p className="muted">
              Esclude appuntamenti cancellati e non presentati.
            </p>
          </div>
          <div className="agenda-revenue-by-staff">
            {expected.byStaff.map((item) => (
              <p key={item.name}>
                <span>{item.name}</span>
      ...
      {data.view === "year" ? (
        <div className="year-calendar">
          {Array.from({ length: 12 }, (_, index) => {
            ...
            const monthEntries = data.entries.filter((entry) =>
              dayForEntry(entry).startsWith(monthKey),
            );
            const activeEntries = monthEntries.filter(
              (entry) => !["CANCELLED", "NO_SHOW"].includes(entry.status),
            );
            ...
            return (
              ...
              >
                ...
                <small>
                  appuntament{monthEntries.length === 1 ? "o" : "i"}
                </small>
                <em>{money(total)}</em>
              </button>
            );
            ...
            return (
              ...
              >
                ...
                <span className="month-day-count">
                  {entries.length
                    ? `${entries.length} appuntament${entries.length === 1 ? "o" : "i"}`
                    : "Nessun appuntamento"}
                </span>
                <span className="month-day-entries">
                  {entries.slice(0, 3).map((entry) => (
                    <span
                      className={`month-entry status-${entry.status.toLowerCase()}`}
                      key={entry.id}
                    >
                      <strong>{time(entry.startsAt)}</strong>{" "}
                      {entry.customerName}
                      <small>{entry.staffName}</small>
                    </span>
                  ))}
                  {entries.length > 3 ? (
                    <span className="month-more">
                      + {entries.length - 3} altri
                    </span>
                  ) : null}
                </span>
              </button>
        ...
        <div className="week-calendar">
          {Array.from({ length: 7 }, (_, index) =>
            addCalendarDays(data.startDate, index),
          ).map((day) => (
            <section
            ...
            >
              <button
                ...
                type="button"
                onClick={() => openDay(day)}
                aria-label={`Apri la giornata del ${day}`}
              >
                <span>
                  {new Intl.DateTimeFormat("it-IT", {
                    weekday: "short",
              ...
              {data.entries
                .filter((entry) => dayForEntry(entry) === day)
                .map((entry) => (
                  <article
                    className={`agenda-appointment status-${entry.status.toLowerCase()}`}
                    key={entry.id}
                  >
                    ...
                    <small className="agenda-price">
                      {money(
                        Number(entry.price) +
                          entry.productTotal +
                          entry.additionalServiceTotal,
                      )}
                    </small>
                    {entry.notes ? (
                      <small className="agenda-card-note">{entry.notes}</small>
                    ) : null}
                    {entry.absenceConflict ? (
                      <strong className="agenda-absence-warning">
                        ⚠ Conflitto assenza
                      </strong>
                    ) : null}
                  </article>
        ...
        <div className="agenda-scroll">
          <div
            ...
            style={{
              gridTemplateColumns: `76px repeat(${Math.max(data.staff.length, 1)}, minmax(190px,1fr))`,
            }}
          >
            <div className="calendar-corner">Ora</div>
            {data.staff.map((member) => (
              <div className="staff-heading" key={member.id}>
                {member.name}
              </div>
            ))}
            {dayTimes.length === 0 ? (
            ...
            {dayTimes.map((slotTime) => {
              return (
                <div
                  ...
                  style={{
                    gridColumn: "1 / -1",
                    gridTemplateColumns: `76px repeat(${Math.max(data.staff.length, 1)}, minmax(0,1fr))`,
                  }}
                >
                  <time>{slotTime}</time>
                  {data.staff.map((member) => (
                    <div className="calendar-cell" key={member.id}>
                      {data.entries
                        .filter(
                          (entry) =>
                            entry.staffId === member.id &&
                            time(entry.startsAt) === slotTime,
                        )
                        .map((entry) => (
                          <article
                            className={`agenda-appointment status-${entry.status.toLowerCase()}`}
                          ...
                          >
                            ...
                            <div className="agenda-total">
                              ...
                              0 ? (
                                <small>
                                  Base {money(Number(entry.price))}
                                  {entry.additionalServiceTotal > 0
                                    ? ` + servizi ${money(entry.additionalServiceTotal)}`
                                    : ""}
                                  {entry.productTotal > 0
                                    ? ` + prodotti ${money(entry.productTotal)}`
                                    : ""}
                                </small>
                              ) : null}
                            </div>
                            {entry.previousOutstanding > 0 ? (
                              <small className="agenda-outstanding-badge">
                                Sospeso {money(entry.previousOutstanding)} ·
                                Totale{" "}
                                {money(
                                  entry.previousOutstanding +
                                    Number(entry.price),
                                )}
                              </small>
                            ) : null}
                            ...
                            {entry.absenceConflict ? (
                              <strong className="agenda-absence-warning">
                                ⚠ Conflitto con assenza
                              </strong>
                            ) : null}
                            {entry.rememberedNote ? (
                              <p className="agenda-remembered-note">
                                <strong>Nota precedente:</strong>{" "}
                                {entry.rememberedNote}
                              </p>
                            ) : null}
                            {editableStatuses.includes(entry.status) ? (
                              <div className="agenda-action-buttons">
                                <button
                                  type="button"
                                  className="agenda-complete-action"
                                  aria-label="Chiudi prenotazione"
                                  title="Chiudi prenotazione"
                                  onClick={(event) => {
                                    event.stopPropagation();
                                    setCompletionFor(entry.id);
                                    setCompletionNote(
                                      entry.rememberedNote ?? "",
                                    );
                                    setCompletionPayment("PAID");
                                    setFailureFor("");
                                  }}
                                >
                                  ✓
                            ...
                            {editFor === entry.id ? (
                              <section
                                className="agenda-action-panel agenda-booking-modal"
                              ...
                              >
                                ...
                                {data.canEditAppointments &&
                                editableStatuses.includes(entry.status) ? (
                                  <details className="agenda-modal-section">
                                    <summary>
                                      Modifica servizio e durata
                                    </summary>
                                    <EditServiceForm
                                      data={data}
                                      entry={entry}
                                ...
                                {data.canManage ? (
                                  <details className="agenda-modal-section">
                                    <summary>Modifica prezzo</summary>
                                    <AppointmentPriceEditor
                                ...
                                {editableStatuses.includes(entry.status) ? (
                                  <details className="agenda-modal-section">
                                    <summary>Servizi aggiuntivi</summary>
                                ...
                                !["CANCELLED", "NO_SHOW"].includes(
                                  ...
                                  <details className="agenda-modal-section">
                                    <summary>Prodotti e articoli</summary>
                                    <ProductPanel
                                      data={data}
                                      entry={entry}
                                      close={() => setEditFor("")}
                                ...
                                {editableStatuses.includes(entry.status) ? (
                                  <div className="agenda-modal-status">
                                    <button
                                      type="button"
                                      className="agenda-complete-action"
                                      onClick={() => {
                                    ...
                                    <button
                                      type="button"
                                      className="agenda-failure-action"
                                      onClick={() =>
                                        setFailureFor(
                                          failureFor === entry.id
                                            ? ""
                                ...
                                {failureFor === entry.id ? (
                                  <div className="agenda-failure-reasons agenda-inline-panel">
                                    <button
                                      type="button"
                                      onClick={() =>
                                        void updateStatus(entry.id, "CANCELLED")
                                      }
                                    >
                                      Cancellato
                                    </button>
                                    <button
                                      type="button"
                                      onClick={() =>
                                        void updateStatus(entry.id, "NO_SHOW")
                                      }
                                    >
                                      Non presentato
                                    </button>
                            ...
                            {completionFor === entry.id ? (
                              <div className="agenda-completion-note agenda-inline-panel">
                                <label>
                                  Note del trattamento
                                  <textarea
                                    value={completionNote}
                                    maxLength={500}
                                    placeholder="Prodotti usati, preferenze, risultato…"
                                    onChange={(event) =>
                                ...
                                {entry.rememberedNote ? (
                                  <small>
                                    È stata precaricata la nota precedente di
                                    questo cliente per {entry.serviceName}.
                                  </small>
                                ) : null}
                                {entry.previousOutstanding > 0 ? (
                                  <div className="agenda-payment-summary">
                                    <span>Sospeso precedente</span>
                                    <strong>
                                      {money(entry.previousOutstanding)}
                                    </strong>
                                    <span>Servizio corrente</span>
                                    <strong>
                                ...
                                {data.canManage ? (
                                  <label>
                                    ...
                                    >
                                      ...
                                      <option value="UNPAID">In sospeso</option>
                                    </select>
                                  </label>
                                ) : (
                                  <small>
                                    Il pagamento sarà registrato come in sospeso
                                    e potrà essere verificato dal titolare.
                                  </small>
                                )}
                                <div>
                                  <button
                                    type="button"
                                    className="agenda-note-confirm"
                                    onClick={() =>
                                      void updateStatus(
                                        entry.id,
                                        "COMPLETED",
