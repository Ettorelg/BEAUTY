...
import { addAbsence, addWorkingHours, assignCategory, assignService, createOwnerStaffProfile, createStaffMember, deleteStaffMember, deleteWorkingHours, inviteStaffMember, removeAssignedService, saveWorkingDayState, updateStaffMember, updateWorkingHours } from "./actions";
...
export default async function StaffPage() {
  ...
  return <main className="dashboard-shell"><AppNav businessName={context.businessName} role={context.role}/>
    ...
    {staff.length ? <details className="panel action-disclosure"><summary><span><b>Turni giornalieri</b><small>Configura orari e turnazioni</small></span><i>＋</i></summary><div className="action-disclosure-body"><WorkingDayForm action={saveWorkingDayState} staff={staff.map(({ id, name }) => ({ id, name }))} weekdays={weekdays}/></div></details> : null}</section>
    {staff.length ? <section className="management-grid compact-management-grid"><details className="panel action-disclosure"><summary><span><b>Associa servizi</b><small>Imposta le competenze dello staff</small></span><i>＋</i></summary><div className="action-disclosure-body">{catalog.length ? <form action={assignService} className="compact-form stacked"><select name="staffId">{staff.map(s=><option value={s.id} key={s.id}>{s.name}</option>)}</select><select name="serviceId">{catalog.map(s=><option value={s.id} key={s.id}>{s.name}</option>)}</select><button className="primary-button">Abilita servizio</button></form> : <p className="muted">Crea prima un servizio.</p>} {categories.length ? <form action={assignCategory} className="compact-form stacked category-assignment"><select name="staffId">{staff.map(s=><option value={s.id} key={s.id}>{s.name}</option>)}</select><select name="categoryId">{categories.map(c=><option value={c.id} key={c.id}>{c.name}</option>)}</select><button className="ghost-button">Abilita intera categoria</button></form> : null}</div></details>
    <details className="panel action-disclosure"><summary><span><b>Registra assenza</b><small>Blocca una fascia non disponibile</small></span><i>＋</i></summary><div className="action-disclosure-body"><AbsenceForm action={addAbsence} staff={staff.map(({ id, name }) => ({ id, name }))}/></div></details></section> : null}
