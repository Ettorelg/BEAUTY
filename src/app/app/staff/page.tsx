...
import { addAbsence, addWorkingHours, assignCategory, assignService, createOwnerStaffProfile, createStaffMember, deleteStaffMember, deleteWorkingHours, removeAssignedService, updateStaffMember, updateWorkingHours } from "./actions";
...
export default async function StaffPage() {
  ...
  return <main className="dashboard-shell"><AppNav businessName={context.businessName} role={context.role}/>
    ...
    <section className="list-section"><h2>Operatori</h2>{staff.length ? <div className="data-list">{staff.map(member => {
      ...
      return <article className="data-row staff-row" key={member.id}><div className="staff-card-main"><h3>{member.name} {isOwnerOperator ? <span className="invite-badge accepted">Titolare</span> : null}</h3><p className="muted">{member.title || "Operatore"}</p>
      ...
      </div><form action={deleteStaffMember} className="delete-form"><input type="hidden" name="id" value={member.id}/><ConfirmSubmitButton message={isOwnerOperator ? "Rimuovere il titolare dall’agenda? L’account amministrativo e l’accesso al salone resteranno attivi." : `Eliminare ${member.name}? Verranno rimossi anche servizi associati, turni e assenze.`}>{isOwnerOperator ? "Rimuovi dall’agenda" : "Elimina operatore"}</ConfirmSubmitButton></form></article>;
