...
export function AppNavLinks({ staffOnly,modules,businessType }: { staffOnly: boolean; modules: BusinessModule[]; businessType: BusinessType }) {
  ...
  })}</nav><footer className="app-side-menu-footer"><PwaInstallButton/>{staffOnly ? <LogoutButton/> : <Link href="/app/settings"><span aria-hidden="true">⚙</span>Account ed Esci in Configurazione</Link>}</footer></aside></div>:null}</>;
}
