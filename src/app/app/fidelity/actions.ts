...
export async function addPromotion(f:FormData){const c=await owner();const x=z.object({serviceId:z.string().uuid(),discountPercent:z.coerce.number().int().min(1).max(100),startsAt:z.coerce.date(),endsAt:z.coerce.date()}).parse(Object.fromEntries(f));if(x.endsAt<=x.startsAt)throw Error("La fine deve essere successiva all’inizio.");const [service]=await db.select({id:services.id}).from(services).where(and(eq(services.id,x.serviceId),eq(services.businessId,c.businessId),eq(services.active,true))).limit(1);if(!service)throw Error("Servizio non valido.");await db.insert(fidelityPromotions).values({businessId:c.businessId,...x});refresh();redirect("/app/fidelity?promo=1")}
