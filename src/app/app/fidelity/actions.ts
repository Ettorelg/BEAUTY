...
export async function saveFidelitySettings(f:FormData){const c=await owner();const x=z.object({spendEuros:z.coerce.number().min(1),pointsAward:z.coerce.number().int().min(1),pointsValidityMonths:z.coerce.number().int().min(1).max(60),allowRewardStacking:z.string().optional()}).parse(Object.fromEntries(f));const settings={spendCents:Math.round(x.spendEuros*100),pointsAward:x.pointsAward,pointsValidityMonths:x.pointsValidityMonths,allowRewardStacking:x.allowRewardStacking==="on",updatedAt:new Date()};await db.insert(fidelitySettings).values({businessId:c.businessId,...settings}).onConflictDoUpdate({target:fidelitySettings.businessId,set:settings});refresh();redirect("/app/fidelity?saved=1")}
