...
export async function GET(request: NextRequest) {
  await ensureFidelitySchema(); const context = await requireBusinessContext(); const isOwner = context.role === "OWNER";
  ...
  const [staff, catalog, entries] = await Promise.all([
    ...
    db.select({ id: appointments.id, startsAt: appointments.startsAt, endsAt: appointments.endsAt, status: appointments.status, serviceId: appointments.serviceId, serviceName: appointments.serviceName, customerName: customerRelations.name, staffId: staffMembers.id, staffName: staffMembers.name, price: appointments.price, notes: appointments.notes }).from(appointments).leftJoin(customerRelations, and(eq(appointments.customerRelationId, customerRelations.id), eq(customerRelations.businessId, context.businessId))).innerJoin(staffMembers, and(eq(appointments.staffId, staffMembers.id), eq(staffMembers.businessId, context.businessId))).where(and(eq(appointments.businessId, context.businessId), isOwner ? undefined : eq(appointments.staffId, ownStaff!.id), gte(appointments.startsAt, start), lt(appointments.startsAt, end))).orderBy(asc(appointments.startsAt)),
  ]);
