...
export default async function PublicSalonPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ service?: string; date?: string }> }) {
  ...
  return <main className="public-shell"><header className="public-header"><div><p className="eyebrow">Prenotazione online</p><h1>{business.name}</h1></div><span>Orari in {business.timezone}</span></header>
