...
import { appointmentRescheduleRequests, appointments, staffAbsences, staffMembers, staffServices, workingHours } from "@/db/schema";
...
export async function getPublicAvailability(input: {
  ...
  const [hours, absences, bookings, holds] = await Promise.all([
    ...
    db.select({ staffId: appointmentRescheduleRequests.proposedStaffId, startsAt: appointmentRescheduleRequests.proposedStartsAt, endsAt: appointmentRescheduleRequests.proposedEndsAt }).from(appointmentRescheduleRequests)
      .where(and(eq(appointmentRescheduleRequests.businessId, input.businessId), inArray(appointmentRescheduleRequests.proposedStaffId, staffIds), eq(appointmentRescheduleRequests.status, "PENDING"), gt(appointmentRescheduleRequests.expiresAt, new Date()), lt(appointmentRescheduleRequests.proposedStartsAt, dayEnd), gt(appointmentRescheduleRequests.proposedEndsAt, dayStart))),
  ]);
  ...
  return eligible.flatMap((person) => {
    ...
    const blocked = [...absences.filter((item) => item.staffId === person.id).map((item) => ({ start: toMinutes(item.startsAt), end: toMinutes(item.endsAt) })),
      ...
      ...holds.filter((item) => item.staffId === person.id).map((item) => ({ start: toMinutes(item.startsAt), end: toMinutes(item.endsAt) }))];
